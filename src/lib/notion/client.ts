...
import { DATABASE_ID, NOTION_API_SECRET, NUMBER_OF_POSTS_PER_PAGE, REQUEST_TIMEOUT_MS } from '../../server-constants';
import type {
  Annotation,
  Block,
  Bookmark,
...
export async function getAllPosts(): Promise<Post[]> {
  ...
  while (true) {
    ...
    results = results.concat(res.results as responses.QueryDatabaseResponse['results']);
  ...
  postsCache = results.filter((pageObject) => _validPageObject(pageObject)).map((pageObject) => _buildPost(pageObject));
  return postsCache;
}
